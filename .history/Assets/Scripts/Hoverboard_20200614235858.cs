...
public class Hoverboard : MonoBehaviour
{
  ...
  private bool m_HasTriedMaxBoost = false;
  ...
  {
    m_RigidBody.drag = m_IsGrounded ? 1f : 2f;
    m_RigidBody.mass = m_IsGrounded ? 3f : 20f;
    ...
    foreach (GameObject point in m_HoverboardPoints)
    {
      ...
      if (Physics.Raycast(downRay, out hit, m_HoverboardPointRayDistance))
      {
        ...
        if (hoverError > 0)
        {
          ...
          float lift1 = m_HoverForce * Mathf.Pow((1f - (hit.distance / m_IdealHoverHeight)), 1.7f) * m_HoverDamp;
