    ...
    foreach (GameObject point in m_HoverboardPoints)
    {
      ...
      if (Physics.Raycast(downRay, out hit))
      {
        ...
        if (hoverError > 0)
        {
          ...
          // lift1 += Random.Range(-m_RandomBounceFactor, m_RandomBounceFactor);
          lift1 += Mathf.Sin(m_RandomBounceFactor) * Random.Range(-m_RandomBounceFactor, m_RandomBounceFactor);
          Debug.Log("Sin " + Mathf.Sin(m_RandomBounceFactor) * Random.Range(-m_RandomBounceFactor, m_RandomBounceFactor));
