...
public class Hoverboard : MonoBehaviour
{
  ...
  {
    m_RigidBody.AddForce(vertical * m_MoveForce * transform.forward, ForceMode.Acceleration);
    m_RigidBody.AddTorque(horizontal * m_TorqueForce * Vector3.up, ForceMode.VelocityChange);
    ...
    foreach (GameObject point in m_HoverboardPoints)
    {
      ...
      if (Physics.Raycast(downRay, out hit))
      {
        ...
        if (hoverError > 0)
        {
          ...
          m_RigidBody.AddForceAtPosition(lift1 * Vector3.up, point.transform.position, ForceMode.Impulse);
        }
      }
      else
      {
