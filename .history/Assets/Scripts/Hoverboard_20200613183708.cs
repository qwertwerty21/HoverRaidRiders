...
public class Hoverboard : MonoBehaviour
{
  ...
  // when drifting, the force you add will be
  // DIVIDED by this number, so the HIGHER the slower the drift speed
  // the LOWER the faster the drift speed
  public float m_DriftSpeedFactor = 12f;
  ...
  public void Move(float horizontal, float vertical, bool isDrifting)
  {
    ...
    if (isDrifting)
    {
      m_RigidBody.AddForce(vertical * m_CurrentSpeed * transform.forward / m_DriftSpeedFactor, ForceMode.Impulse);
    }
    else
    {
