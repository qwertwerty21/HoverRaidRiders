...
public class Hoverboard : MonoBehaviour
{
  ...
  public MMFeedbacks m_TestFeedback;
  ...
  {
    DampenAngularVelocity();
    bool fuck = collision.gameObject.layer == Constants.m_GroundLayerMask;
    print("OnCollissionStay" + fuck);

    if (collision.gameObject.layer == Constants.m_GroundLayerMask)
    {
      print("AddForceAtPosition" + collision.gameObject.layer);

      m_RigidBody.AddForceAtPosition(m_AbsoluteMaxLift * Vector3.up, m_HoverboardGroundCheckPoint.transform.position, ForceMode.Impulse);
    }
  }
}
