...
public class Hoverboard : MonoBehaviour
{
  ...
  public float m_GroundCheckSphereRadius = 1f;
  ...
  public void Move(float horizontal, float vertical, bool isDrifting)
  {
    if (m_IsGrounded)
    {
      ...
      else
      {
       ...
       m_RigidBody.AddForce(vertical * m_AirSpeed * transform.forward, ForceMode.Acceleration);
    }
  ...
  private void FixedUpdate()
  {
    ...
    // check if grounded
    Collider[] colliders = Physics.OverlapSphere(m_HoverboardGroundCheckPoint.transform.position, m_GroundCheckSphereRadius, m_GroundLayerMask);
    Debug.Log("Colliders.Length " + colliders.Length);
    m_IsGrounded = colliders.Length > 0f;
