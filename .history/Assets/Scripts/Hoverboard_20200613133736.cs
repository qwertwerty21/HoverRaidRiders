    ...
    foreach (GameObject point in m_HoverboardPoints)
    {
      ...
      if (Physics.Raycast(downRay, out hit))
      {
        ...
        if (hoverError > 0)
        {
          ...
          lift1 += Mathf.Sin(m_RigidBody.velocity * m_HoverBounceSpeed) * m_HoverBounceHeight;
          Debug.Log("Sin " + Mathf.Sin(m_RigidBody.velocity * m_HoverBounceSpeed) * m_HoverBounceHeight);
