...
public class Player : MonoBehaviour
{
  ...
  [SerializeField] private float m_DoubleTapDelay = .2f;
  ...
  private void FixedUpdate()
  {
    ...
    if (canMove)
    {
      ...
      //clamp input values
      float steering = Mathf.Clamp(m_LocomotionInput.x, -1, 1);
      float accel = Mathf.Clamp(m_LocomotionInput.y, 0, 1);
      float footbrake = -1 * Mathf.Clamp(m_LocomotionInput.y, -1, 0);
