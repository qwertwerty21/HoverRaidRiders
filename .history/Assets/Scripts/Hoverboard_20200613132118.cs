...
public class Hoverboard : MonoBehaviour
{
  ...
  [Range(0f, 100f)]
  public float m_RandomBounceSpeed = 1f;
    ...
    foreach (GameObject point in m_HoverboardPoints)
    {
      ...
      if (Physics.Raycast(downRay, out hit))
      {
        ...
        if (hoverError > 0)
        {
          ...
          lift1 += Mathf.Sin(Time.time * m_RandomBounceSpeed) * m_RandomBounceFactor / 2f;
          Debug.Log("Sin " + Mathf.Sin(Time.time * m_RandomBounceSpeed) * m_RandomBounceFactor / 2f);
